...
export class AfterCancelComponent implements OnInit {
  ...
  delete() {
    ...
    this.cTicket = new CancelTicket(this.resv.ticketNo, this.resv.bookingDate, this.resv.noOfSeats, 0, 0, new Date(), this.resv.totalFare, 0, this.resv.emailId, this.resv.phoneNo, this.resv.bus, this.resv.user);

    this.resService.updateReservation(this.cTicket).subscribe(data =>{

    });

    this.busService.getBusByIdDate(this.bid, this.dDate).subscribe(data => {
      this.bus = data;
    });
