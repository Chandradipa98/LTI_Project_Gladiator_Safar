...
export class AdminComponent implements OnInit {
  ...
  addBus(): void {
    ...
    let buses1: Bus = new Bus(this.addBusForm.controls.busId.value, this.addBusForm.controls.source.value,
      ...
      this.addBusForm.controls.arrivalTime.value, this.addBusForm.controls.departureTime.value, this.addBusForm.controls.arrivalDate.value,
      this.addBusForm.controls.departureDate.value, this.addBusForm.controls.availableSeat.value, this.addBusForm.controls.totalSeat.value,
      this.addBusForm.controls.baseFare.value, this.bs, this.cbs)
  ...
  updateBus(): void {
    ...
    let buses1: Bus = new Bus(this.updateBusForm.controls.busId.value, this.updateBusForm.controls.source.value,
      ...
      this.updateBusForm.controls.arrivalTime.value, this.updateBusForm.controls.departureTime.value, this.updateBusForm.controls.arrivalDate.value,
      this.updateBusForm.controls.departureDate.value, this.updateBusForm.controls.availableSeat.value, this.updateBusForm.controls.totalSeat.value,
      this.updateBusForm.controls.baseFare.value, this.bs, this.cbs)
  ...
  removeBus(): void {
    ...
    for (let i = 0; i < this.Bus1.length; i++) {
      if (this.busid == this.Bus1[i].id) {
        BUS = new Bus(this.Bus1[i].id, this.Bus1[i].source, this.Bus1[i].destination, this.Bus1[i].arrivalTime, this.Bus1[i].departureTime, this.Bus1[i].arrivalDate, this.Bus1[i].departureDate,this.Bus1[i].availableSeat,this.Bus1[i].totalSeat,this.Bus1[i].baseFare,this.bs,this.Bus1[i].coachBusStatus);
      }
    }
