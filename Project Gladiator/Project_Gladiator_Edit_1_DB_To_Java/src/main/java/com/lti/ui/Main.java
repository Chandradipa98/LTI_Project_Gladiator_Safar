...
public class Main {
	...
	public static void main(String[] args) {
		...
		BusTbl bus1 = new BusTbl(new Date(), new Date(), "Kolkata", "Pune", 350, 10, 20, 'Y', "10.30 a.m.",
				"10.30 p.m.");
		BusTbl bus2 = new BusTbl(new Date(), new Date(), "Kolkata", "Howrah", 100, 15, 20, 'Y', "11.00 a.m.",
				"12.00 p.m.");
		BusTbl bus3 = new BusTbl(new Date(), new Date(), "Mumbai", "Delhi", 400, 12, 20, 'Y', "1.00 a.m.", "1.00 p.m.");

		Payment pay1 = new Payment(1000, "Google Pay");
		Payment pay2 = new Payment(1000, "Net Banking");
		Payment pay3 = new Payment(1000, "Phone pe");
		Payment pay4 = new Payment(1000, "Paytm");
		Payment pay5 = new Payment(1000, "Wallet");
		...
		pay1.setDayDifference(
				pay1.getRes().getBus().getDeparture().getDate() - pay1.getRes().getBookingDate().getDate());
		pay2.setDayDifference(
				pay2.getRes().getBus().getDeparture().getDate() - pay2.getRes().getBookingDate().getDate());
		pay3.setDayDifference(
				pay3.getRes().getBus().getDeparture().getDate() - pay3.getRes().getBookingDate().getDate());
		pay4.setDayDifference(
				pay4.getRes().getBus().getDeparture().getDate() - pay4.getRes().getBookingDate().getDate());
		pay5.setDayDifference(
				pay5.getRes().getBus().getDeparture().getDate() - pay5.getRes().getBookingDate().getDate());

		pay1.setChargedFare(pay1.getAddedFare() - (pay1.getDayDifference() * 10));
		pay2.setChargedFare(pay2.getAddedFare() - (pay2.getDayDifference() * 10));
		pay3.setChargedFare(pay3.getAddedFare() - (pay3.getDayDifference() * 10));
		pay4.setChargedFare(pay4.getAddedFare() - (pay4.getDayDifference() * 10));
		pay5.setChargedFare(pay5.getAddedFare() - (pay5.getDayDifference() * 10));
		...
		res1.setPayment(pay1);
		res2.setPayment(pay2);
		res3.setPayment(pay3);
		res4.setPayment(pay4);
		res5.setPayment(pay5);

		res1.setTotalFare((res1.getBus().getBaseFare() + res1.getPayment().getChargedFare()) * res1.getNoOfSeats());
		res2.setTotalFare((res2.getBus().getBaseFare() + res2.getPayment().getChargedFare()) * res2.getNoOfSeats());
		res3.setTotalFare((res3.getBus().getBaseFare() + res3.getPayment().getChargedFare()) * res3.getNoOfSeats());
		res4.setTotalFare((res4.getBus().getBaseFare() + res4.getPayment().getChargedFare()) * res4.getNoOfSeats());
		res5.setTotalFare((res5.getBus().getBaseFare() + res5.getPayment().getChargedFare()) * res5.getNoOfSeats());
